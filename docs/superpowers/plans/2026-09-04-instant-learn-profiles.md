...
def test_owner_policy_and_tone_types():
    ...
    assert classify_correction("Keep it shorter and polite", is_owner=True).patch_type == "tone"
    assert classify_correction("Say 'site visits 10-6' like this", is_owner=True).patch_type == "exemplar"
